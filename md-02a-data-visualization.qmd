...
2.  Start with the `gapminder` object and add the pipe operator (command + shift + m) at the end of the line.
...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.** <https://www.statology.org/remove-outliers-in-boxplots-r/>

4.  Run the code contained in the code chunk and fix any errors.

5.  What are the data types of the three variables used for aesthetic mappings? categorical variables (continents) (?)
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: the geom_bar() does not need the y axis, it only needs the x axis compared to the geom_col(), which needs both x- and y-axis. but not exactly sure what else I can read/know from this ... :/
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    The histogram is reader friendly I think because it also uses more colors etc. so you can distinguish better the facts.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    so when I use 10 instead of 2.5. the bars get less accurate and sum up more steps and there are wider assumptions. And when I use e.g. 0.5 instead of 2.5 the bars get smaller, more narrow and more accurate but also it is harder to read and really detailed and not as simple to look at.
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    I think it makes that eack continent stays in a certain color range
